  ...
  try {
    ...
    if (!apiKey) {
      ...
      return res.status(500).json({
        success: false,
        error: "Email service is not configured. Please set RESEND_API_KEY in Vercel or environment variables.",
      });
    }
...
app.all("/api/test-email", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      error: err.message || "An error occurred while sending test email.",
    });
  }
});
