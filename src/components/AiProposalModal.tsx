...
Strategy: ${proposal.growthStrategy}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-3xl bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-800 bg-slate-100 rounded-full border border-slate-200 cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center gap-3">
          <div className="p-1 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center shadow-sm">
            <DrazonLogo variant="icon-only" size="sm" />
          </div>
          <div>
            <h3 className="text-2xl font-black text-slate-900 flex items-center gap-2">
              <span>Instant Proposal Engine</span>
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-50 text-[#059669] border border-emerald-200 font-bold">
                DRAZON.NET
              </span>
            </h3>
            <p className="text-xs text-slate-500">Get a tailored scope, timeline, and strategy generated by Drazon AI</p>
          </div>
        </div>
        ...
        {!proposal ? (
          <form onSubmit={handleGenerate} className="space-y-4 pt-2">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs focus:border-[#10B981] focus:outline-none"
                />
              </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Primary Business Goal</label>
              <input
                type="text"
                value={goals}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-2">Select Key Website Features Needed</label>
                ...
                {featureOptions.map((feat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-xl text-[11px] font-semibold text-left border transition cursor-pointer flex items-center justify-between ${
                        isSel
                    ...
                    >
                      ...
                      {isSel && <Check className="w-3.5 h-3.5 text-[#10B981] shrink-0" />}
                    </button>
                  );
                })}
              </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 px-6 text-xs font-bold uppercase tracking-wider text-white bg-gradient-to-r from-[#10B981] to-[#059669] hover:from-[#059669] hover:to-[#10B981] rounded-xl shadow-md shadow-[#10B981]/25 transition duration-200 cursor-pointer flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          <div className="space-y-6 pt-2 animate-in fade-in duration-300">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold">Recommended Package</p>
                <p className="text-lg font-black text-[#10B981]">{proposal.recommendedPlan}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold">Estimated Turnaround</p>
                <p className="text-base font-bold text-slate-900">{proposal.estimatedTimeline}</p>
              </div>
              <button
                onClick={copyProposal}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-100 rounded-lg border border-slate-200 cursor-pointer shadow-sm"
              >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-700 leading-relaxed p-4 rounded-xl bg-slate-50 border border-slate-200">
            ...
            {/* Deliverables */}
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Project Deliverables</h4>
              <div className="grid sm:grid-cols-2 gap-2">
                {proposal.coreDeliverables.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-2 p-2.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-800">
            ...
            {/* Growth Strategy */}
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Digital Growth Strategy</h4>
              <p className="text-xs text-slate-600 leading-relaxed p-3 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 text-xs font-bold text-slate-500 hover:text-slate-900 cursor-pointer"
              >
